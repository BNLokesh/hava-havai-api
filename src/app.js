...
app.get('/airport', async (req, res) => {
    ...
    try {
      const airport = await Airport.findOne({
        where: { iata_code },
        include: [
          { model: City, as: 'city', include: [{ model: Country, as: 'country' }] }
        ]
      });
      ...
      let response = {
        ...
        address: {
          ...
          country: null // Initialize with null, might get overridden
        }
      };
  
      // If airport has a country associated with its city, update the response
